...
export default function RecordList() {
  ...
  const filteredRecords = records.filter(rec => {
    ...
    // Ensure 'rec.name', 'rec.position', and 'rec.level' are not null or undefined
    const isNameMatch = rec.name && rec.name.toLowerCase().includes(lowerCaseSearchTerm);
    const isPositionMatch = rec.position && rec.position.toLowerCase().includes(lowerCaseSearchTerm);
    const isLevelMatch = rec.level && rec.level.toLowerCase().includes(lowerCaseSearchTerm);
  
    return isNameMatch || isPositionMatch || isLevelMatch;
  });
